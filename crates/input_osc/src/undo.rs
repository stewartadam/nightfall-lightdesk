...
use crate::{command::OscMapping, mapping::OscMappings};
...
impl MappingChange {
    ...
    fn proposed(&self, mappings: &OscMappings) -> Result<Vec<OscMapping>, CommandError> {
        let id = self
            .expected
            .as_ref()
        ...
        if let Some(mapping) = &self.replacement {
            if proposed
                .iter()
                .any(|other| OscMappings::bindings_conflict(mapping, other))
            {
                return Err(CommandError::new(
...
impl UndoableOperation for MappingChange {
    ...
    fn inverse(&self, ctx: &UndoContext) -> Option<Box<dyn UndoableOperation>> {
        self.proposed(ctx.world.resource::<OscMappings>()).ok()?;
        Some(Box::new(Self {
...
pub(crate) fn record(
    ...
    undo_id: UndoId,
    before: Option<OscMapping>,
    after: Option<OscMapping>,
) {
    manager.push(
        UndoEntry {
            ...
            description: "Map OSC controller".into(),
...
fn replay(
    ...
    for event in events.read() {
        ...
        if let Err(error) = result {
            tracing::error!(%error, "osc_mapping_replay_completion_failed");
        }
    }
}
...
mod tests {
    ...
    fn learn(app: &mut App, action: &str, replace: Option<OscMapping>) -> OscMapping {
        ...
        assert!(
            app.world_mut()
                ...
                .capture(LearnedControllerSource {
                    surface: ActionSurface::Osc,
                    selector: serde_json::json!({"address":"/test", "arg_index":0}),
    ...
    fn learned_mapping_undo_redo_retains_identity_and_rejects_stale_state() {
        ...
        unrelated.address = "/unrelated".into();
        app.world_mut()
        ...
        assert!(
            matches!(submit(&mut app, OscCommand::StoreMapping { expected: Some(first.clone()), mapping: first.clone() }), CommandOutcome::Failed(error) if error.code == "osc.mapping_changed")
        );
        assert!(
            matches!(submit(&mut app, OscCommand::StoreMapping { expected: None, mapping: second.clone() }), CommandOutcome::Failed(error) if error.code == "osc.mapping_changed")
        );
        assert_eq!(
            submit(
                &mut app,
                OscCommand::RemoveMapping {
        ...
        let mut current = app.world().resource::<OscMappings>().mappings().to_vec();
        ...
        assert_eq!(app.world().resource::<UndoManager>().undo_depth(), depth);
        assert_eq!(app.world().resource::<OscMappings>().mappings(), current);
    }
}
