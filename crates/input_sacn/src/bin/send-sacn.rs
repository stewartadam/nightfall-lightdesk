...
fn main() {
    ...
    let mut source = SacnSource::with_ip("nightfall-input-test", bind_addr)
        .expect("failed to create sACN source");

    source
        .register_universe(config.universe)
        .expect("failed to register sACN universe");

    let mut chase_index = 0usize;

    loop {
        let mut data = vec![0u8; 513];
        apply_pattern(&mut data[1..], &config, &mut chase_index);

        let dst_ip = config
            .target_ip
            .map(|ip| SocketAddr::new(IpAddr::V4(ip), ACN_SDT_MULTICAST_PORT));

        source
            .send(&[config.universe], &data, Some(0), dst_ip, None)
...
const USAGE: &str = "Usage: send-sacn [--universe N] [--ip A.B.C.D] [--fps N] [--pattern solid:V|ramp|chase] [--channels START-END]";
...
impl Config {
        ...
        I: Iterator<Item = String>,
    {
        ...
        if !(1..=63_999).contains(&self.universe) {
            return Err("universe must be within 1-63999".to_string());
        }
