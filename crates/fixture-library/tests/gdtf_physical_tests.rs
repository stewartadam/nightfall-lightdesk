...
fn linear_mapping_round_trips_full_resolution() {
    ...
    let physical = compile_physical(&channels, compile_profiles(&[], 0).unwrap(), 100).unwrap();
...
fn curve_scales_into_explicit_physical_limits() {
    ...
    let channels = resolve_functions(&mode).unwrap();
    let physical =
        compile_physical(&channels, compile_profiles(&profiles, 1).unwrap(), 100).unwrap();
...
fn malformed_mapping_and_budget_are_rejected() {
    ...
    let channels = resolve_functions(&mode).unwrap();
    assert_eq!(
        compile_physical(&channels, compile_profiles(&[], 0).unwrap(), 100)
            .unwrap_err()
            .code,
        "missing_profile"
    );
    assert_eq!(
        compile_physical(&channels, compile_profiles(&[], 0).unwrap(), 0)
            .unwrap_err()
            .code,
        "function_limit"
    );
