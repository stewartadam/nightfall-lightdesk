...
/// Resolves a sequence reference from this domain's authoritative storage.
fn lookup_sequence(
    In(reference): In<ObjectRef>,
    provider: Res<DataProvider<cue::Sequence>>,
) -> ObjectLookupResult {
...
pub(crate) fn register_object_lookups(app: &mut App) {
    register_object_lookup(app.world_mut(), ObjectType::Sequence, lookup_sequence);
}
...
mod tests {
    ...
    fn registered_provider_objects_follow_stored_ids() {
        ...
        let mut object = cue::Sequence::default();
        object.identifiers.id = 7;
        let uid = object.identifiers.uid;
        app.world_mut()
            .resource_mut::<DataProvider<cue::Sequence>>()
            .add(object.clone())
            .unwrap();
        assert!(
            matches!(resolve_object(app.world_mut(), &ObjectRef::ById { object_type: ObjectType::Sequence, id: 7 }), Ok(ObjectIdentity { object_type: ObjectType::Sequence, uid: found }) if found == uid)
        );
        ...
        assert!(
            matches!(resolve_object(app.world_mut(), &ObjectRef::ById { object_type: ObjectType::Sequence, id: 8 }), Ok(ObjectIdentity { object_type: ObjectType::Sequence, uid: found }) if found == uid)
        );
    }
}
