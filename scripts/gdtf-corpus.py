...
def run_probe(executable, archive, report, timeout, expected_modes):
    ...
    for stage in ("resolution", "wire", "functions", "sets", "physical", "relations", "bindings", "activation", "compiled_channels", "conversion"):
    ...
    if args.probe:
        ...
        for result in results:
            ...
            result["function_acceptance"] = check_function_expectations(
                Path(result["nightfall"]["report"]), cases)
    scope = ("archive identity, XML inventory, Rust resolution/wire/functions/sets/physical/relations/bindings/activation/compiled_channels/conversion, and selected targets"
