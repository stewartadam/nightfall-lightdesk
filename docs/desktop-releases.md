...
1. Set the same version in `Cargo.toml` under `[workspace.package]` and in `crates/app-tauri/tauri.conf.json`. Update `Cargo.lock` through Cargo and merge the version change after validation. Include a `Notes:` declaration on every PR, including version-only PRs; see [PR release notes](contributing/release-notes.md). Preserve commit ancestry when promoting `develop` to `main` so the release includes the original PR notes.
