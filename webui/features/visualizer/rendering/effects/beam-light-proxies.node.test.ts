...
/** Verifies a single-beam proxy keeps its gobo and cone, while merged beams drop the gobo and widen. */
test("single beams keep gobos and merged cones cover their members", () => {
  const gobo = { name: "stars" };
  const [single] = buildBeamLightProxies([pixel(0, [1, 1, 1], { gobo })]);
  assert.equal(single.gobo, gobo);
  near(single.halfAngle, 0.1, "single cone unchanged");
  ...
  assert.equal(merged.gobo, undefined);
  assert.ok(
